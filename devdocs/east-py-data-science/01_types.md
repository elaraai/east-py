...
from east.types.values import (
    ...
    east_null, EastSome, EastNone, is_east_option,
)
```
...
NamedHyperparamType = StructType([
    ("name", StringType),
    ("value", HyperparamValueType),
])
...
TrialResultType = StructType([
    ...
    ("params", ArrayType(NamedHyperparamType)),
    ("score", FloatType),
])

# Optimization study result
StudyResultType = StructType([
    ("best_params", ArrayType(NamedHyperparamType)),
...
def _get_option(value: Any, default: Any) -> Any:
    """Extract value from Option (EastSome/EastNone) or return default.

    Uses the existing is_east_option helper from east-py.
    """
    if is_east_option(value):
        if value.is_some():
            return value.value
        else:
            return default
    # Not an option type, return as-is (or default if None)
