"""Tests that run TypeScript-exported IR tests for east-py-std.

This module loads IR test files exported from the TypeScript east-node-std package
via `npm run test:export` and executes them in Python to verify cross-implementation
compatibility.

To generate the test IR files:
    cd ../east-node/packages/east-node-std && npm run test:export
"""
...
TEST_IR_DIR = Path("/tmp/east-node-std")

# Log file for detailed errors
ERROR_LOG_DIR = Path("/tmp/east-py-std-test-errors")

# Profiling output directory
PROFILING_DIR = Path("/tmp/east-py-std-profiling")
...
def get_test_ir_files():
    ...
    if not TEST_IR_DIR.exists():
        raise FileNotFoundError(
            ...
            "Run 'cd ../east-node/packages/east-node-std && npm run test:export' to generate test files."
        )
...
def test_platforms(subtests):
    """Platform functions for tests - combines python_platform with test tracking."""
    ...
    # Combine with python_platform (excluding the default test_impl)
    # Filter out test functions from python_platform to use our tracking versions
    # Note: PlatformFunction is a TypedDict, so access via ["name"] not .name
    platform_fns = [
        pf
        for pf in python_platform
