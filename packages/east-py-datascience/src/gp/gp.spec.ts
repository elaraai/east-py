...
describeEast("GP platform functions", (test) => {
    ...
    test("predict_std returns uncertainty", $ => {
        ...
        $(Assert.equal(result.mean.size(), 5n));
        $(Assert.equal(result.std.size(), 5n));
    ...
    test("uncertainty increases away from training data", $ => {
        ...
        const y_train = $.let([1.0, 4.0, 9.0, 16.0, 25.0]);
    ...
    test("error: train shape mismatch", $ => {
        const X = $.let([[1.0], [2.0], [3.0]]);  // 3 samples
        const y = $.let([1.0, 4.0]);  // 2 samples
