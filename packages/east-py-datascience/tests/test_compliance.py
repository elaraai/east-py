...
#
"""Tests that run TypeScript-exported IR tests for east-py-datascience.

This module loads IR test files exported from the TypeScript east-py-datascience package
...
ERROR_LOG_DIR = Path("/tmp/east-py-datascience-test-errors")

# Profiling output directory
PROFILING_DIR = Path("/tmp/east-py-datascience-profiling")
...
def get_test_ir_files():
    ...
    if not TEST_IR_DIR.exists():
        raise FileNotFoundError(
            ...
            "Run 'npm run test:export' in packages/east-py-datascience to generate test files."
        )
...
def test_typescript_exported_ir(test_file, test_platforms):
    ...
    try:
        ...
        assert (
            len(executed_tests) > 0
        ), f"Test {test_file.stem} didn't execute any test cases"
        ...
        profile_file = (
            PROFILING_DIR
            / f"{test_file.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        if failures:
            log_file = (
                ERROR_LOG_DIR
                / f"{test_file.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
            )
        ...
        log_file = (
            ERROR_LOG_DIR
            / f"{test_file.stem}_CRASH_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        with open(log_file, "w") as f:
            f.write(f"TypeScript Test: {test_file.stem}\n")
