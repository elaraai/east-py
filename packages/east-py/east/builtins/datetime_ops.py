...
def datetime_print_format(dt: datetime, tokens: list[EastValue]) -> str:
...
def datetime_parse_format(text: str, tokens: list[EastValue]) -> datetime:
    ...
    if result["success"]:
        # Type narrowing: result is ParseSuccess
        return result["value"]  # type: ignore[typeddict-item]
    # Format error with position - result is ParseError
    error_msg = result["error"]  # type: ignore[typeddict-item]
    position = result["position"]  # type: ignore[typeddict-item]
