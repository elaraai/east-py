...
from east.types.values import EastBlob, EastValue


def blob_length(b: EastBlob) -> int:
    """Get length of blob."""
    return len(b.data)


def blob_get(b: EastBlob, index: int) -> int:
    """Get byte at index."""
    return b.data[index]
...
def string_to_blob(s: str) -> EastBlob:
    """Encode string as UTF-8 blob."""
    return EastBlob(s.encode("utf-8"))


def string_encode_utf16(s: str) -> EastBlob:
...
def blob_encode_beast_for(T: EastType) -> Callable[[EastValue], EastBlob]:
    ...
    def blob_encode_beast(value: EastValue) -> EastBlob:
        return EastBlob(encoder(value))
...
def blob_encode_beast2_for(T: EastType) -> Callable[[EastValue], EastBlob]:
    ...
    def blob_encode_beast2(value: EastValue) -> EastBlob:
        return EastBlob(encoder(value))
