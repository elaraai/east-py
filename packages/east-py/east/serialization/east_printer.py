...
def _print_east_internal(
    ...
    if (
        is_array_type(value_type)
        or is_set_type(value_type)
        or is_dict_type(value_type)
        or is_ref_type(value_type)
        or is_struct_type(value_type)
    ):
        value_id = id(value)
    ...
    if is_null_type(value_type):
        return print_null(value)
    if is_boolean_type(value_type):
    ...
    if is_recursive_type(value_type):
...
def print_null(_value: Any) -> str:
    """Print east_null value.

    Args:
        _value: east_null value (unused)
...
def print_blob(value: EastBlob) -> str:
    """Print blob value.

    Args:
        value: EastBlob value

    Returns:
        Hex string (0x...)
    """
    hex_str = value.data.hex()
    return f"0x{hex_str}"
...
def print_set(value: EastSet, set_type: SetTypeAlias) -> str:
...
def print_dict(value: EastDict, dict_type: DictTypeAlias) -> str:
...
def print_ref(value: EastRef, ref_type: RefTypeAlias) -> str:
    """Print ref value (no alias tracking).

    Args:
        value: EastRef instance
        ref_type: Ref type

    Returns:
        Ref as text
    """
    from east.types.values import deref
...
def print_struct(value: EastStruct, struct_type: StructTypeAlias) -> str:
...
def print_variant(value: EastVariant, variant_type: VariantTypeAlias) -> str:
    ...
    # Print value (if not east_null)
    if not isinstance(val, EastNull):
...
def print_type(type_val: EastType, stack: list[EastType] | None = None) -> str:
    ...
    if is_array_type(type_val):
        ...
        ret = f".Array {print_type(elem_type, stack)}"
        stack.pop()
        return ret
    ...
    if is_ref_type(type_val):
        stack.append(type_val)
        elem_type = type_val.value
        ret = f".Ref {print_type(elem_type, stack)}"
        stack.pop()
        return ret
