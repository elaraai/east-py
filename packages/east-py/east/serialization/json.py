...
- Special handling for Integer (as string), Float (special values), DateTime (RFC 3339), EastBlob (hex)
...
def _find_recursive_scope_ids(typ: EastType, found: set[int]) -> None:
    """Find all recursive scope IDs in a type."""
    if not hasattr(typ, "type"):
        return

    if is_recursive_type(typ):
        found.add(typ.value)
    elif is_array_type(typ) or is_set_type(typ) or is_ref_type(typ):
    ...
    def find_all_markers(t: EastType, markers: set[Any]) -> None:
        ...
        if is_recursive_type(t):
            ...
            if isinstance(marker, RecursiveTypeMarker):
                markers.add(marker)
        elif is_array_type(t) or is_set_type(t) or is_ref_type(t):
...
def to_json_for(
    type_val: EastType, type_ctx: list[Any] | None = None, marker_map: dict[Any, int] | None = None
) -> Any:
...
def _build_json_encoder(type_val: EastType, type_ctx: list[Any], marker_map: dict[Any, int]) -> Any:
    ...
    if is_blob_type(type_val):
        ...
        def encode_blob(blob: EastBlob, _ctx=None):
    ...
    if is_ref_type(type_val):
        ...
        def encode_ref(r: EastRef, ctx: JSONEncodeValueContext | None = None):
            if ctx is None:
                ctx = JSONEncodeValueContext()
            ref_id = id(r)
    ...
    if is_struct_type(type_val):
        ...
        def encode_struct(obj: EastStruct, ctx: JSONEncodeValueContext | None = None):
            if ctx is None:
                ctx = JSONEncodeValueContext()
            result: dict[str, Any] = {}
    ...
    if is_variant_type(type_val):
        ...
        def encode_variant(variant: EastVariant, ctx: JSONEncodeValueContext | None = None):
            if ctx is None:
                ctx = JSONEncodeValueContext()
            variant_tag = variant.type
            variant_value = variant.value
            ctx.current_path.append(variant_tag)
            encoded_value = case_encoders[variant_tag](variant_value, ctx)
            ctx.current_path.pop()
            return {"type": variant_tag, "value": encoded_value}
...
def _build_json_decoder(
    ...
    if is_array_type(type_val):
        ...
        value_type_str = print_type(type_val.value)
    ...
    if is_set_type(type_val):
        ...
        element_type_str = print_type(type_val.value)
    ...
    if is_dict_type(type_val):
        ...
        key_type_str = print_type(type_val.value["key"])
        value_type_str = print_type(type_val.value["value"])
        key_decoder = from_json_for(
            type_val.value["key"],
            frozen,
            type_ctx,
    ...
    if is_ref_type(type_val):
        ...
        def decode_ref(json_val, ctx: JSONDecodeValueContext | None = None):
            ...
            # Create east_ref and pre-register
            r = east_ref(None)  # Placeholder
            path_key = "/" + "/".join(encode_json_pointer_component(c) for c in ctx.current_path)
            ctx.refs[path_key] = r
            ...
            try:
                ...
                from east.types.values import set_ref
        ...
        inner_type_str = print_type(type_val.value)
