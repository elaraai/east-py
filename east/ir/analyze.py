...
from east.types.types import EastType, IRNode, Location
...
def analyze_ir(
    ir: IRNode, platform: list[PlatformFunction], ctx: VariableContext | None = None
) -> tuple[IRNode, dict[int, bool]]:
    ...
    def visit_ir(node: IRNode, var_ctx: VariableContext) -> bool:
        ...
        if tag == "Platform":
            ...
            # Check if the function type has async platforms
            func_type = node["value"]["function"]["value"]["type"]
            if func_type["type"] == "Function":
                for platform_name in func_type["value"]["platforms"]:
                    if platform_name in platform_map and platform_map[platform_name]:
                        is_async = True
                        break
            ...
            for case in node["value"]["cases"]:
                # Create child context for case body with case variable
                child_ctx = var_ctx.copy()

                # Add case variable to child context
                case_var = case["variable"]["value"]
                child_ctx[case_var["name"]] = VariableMetadata(
                    type=case_var["type"],
                    mutable=case_var["mutable"],
                    defined_by=node,  # Defined by this Match node
                    defined_at=case_var["location"],
                    captured=False,
                )

                # Visit case body with child context
                if visit_ir(case["body"], child_ctx):
                    is_async = True
            ...
            for field in node["value"]["fields"]:
                if visit_ir(field["value"], var_ctx):
                    is_async = True
            ...
            # Create child context for loop body with key and value variables
            child_ctx = var_ctx.copy()

            # Add key variable (index) to child context
            key_var = node["value"]["key"]["value"]
            child_ctx[key_var["name"]] = VariableMetadata(
                type=key_var["type"],
                mutable=key_var["mutable"],
                defined_by=node,  # Defined by this ForArray node
                defined_at=key_var["location"],
                captured=False,
            )

            # Add value variable (element) to child context
            val_var = node["value"]["value"]["value"]
            child_ctx[val_var["name"]] = VariableMetadata(
                type=val_var["type"],
                mutable=val_var["mutable"],
                defined_by=node,  # Defined by this ForArray node
                defined_at=val_var["location"],
                captured=False,
            )

            # Visit body with child context that has key and value variables
            if visit_ir(node["value"]["body"], child_ctx):
                is_async = True
            ...
            # Create child context for loop body with key variable
            child_ctx = var_ctx.copy()

            # Add key variable (element) to child context
            key_var = node["value"]["key"]["value"]
            child_ctx[key_var["name"]] = VariableMetadata(
                type=key_var["type"],
                mutable=key_var["mutable"],
                defined_by=node,  # Defined by this ForSet node
                defined_at=key_var["location"],
                captured=False,
            )

            # Visit body with child context that has key variable
            if visit_ir(node["value"]["body"], child_ctx):
                is_async = True
            ...
            # Add key variable to child context
            key_var = node["value"]["key"]["value"]
            child_ctx[key_var["name"]] = VariableMetadata(
                type=key_var["type"],
                mutable=key_var["mutable"],
                defined_by=node,  # Defined by this ForDict node
                defined_at=key_var["location"],
                captured=False,
            )

            # Add value variable to child context
            val_var = node["value"]["value"]["value"]
            child_ctx[val_var["name"]] = VariableMetadata(
                type=val_var["type"],
                mutable=val_var["mutable"],
            ...
            # Visit try body with current context
            if visit_ir(node["value"]["try_body"], var_ctx):
                is_async = True

            # Create child context for catch body with message and stack variables
            catch_ctx = var_ctx.copy()

            # Add message variable to catch context
            msg_var = node["value"]["message"]["value"]
            catch_ctx[msg_var["name"]] = VariableMetadata(
                type=msg_var["type"],
                mutable=msg_var["mutable"],
                defined_by=node,  # Defined by this TryCatch node
                defined_at=msg_var["location"],
                captured=False,
            )

            # Add stack variable to catch context
            stack_var = node["value"]["stack"]["value"]
            catch_ctx[stack_var["name"]] = VariableMetadata(
                type=stack_var["type"],
                mutable=stack_var["mutable"],
                defined_by=node,  # Defined by this TryCatch node
                defined_at=stack_var["location"],
                captured=False,
            )

            # Visit catch body with catch context that has message and stack variables
            if visit_ir(node["value"]["catch_body"], catch_ctx):
                is_async = True

            # Mark captured variables in parent context
            for var_name, var_meta in catch_ctx.items():
                if var_name in var_ctx and var_meta is var_ctx[var_name]:
                    var_ctx[var_name].captured = True
